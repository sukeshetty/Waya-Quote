...
const App: React.FC = () => {
  ...
  // =====================================================
  // NEW: Handle image updates from QuotationPreview
  // When user uploads an image for a hotel, destination, etc.
  // this updates the quotation data so it persists and exports correctly.
  // =====================================================
  const handleUpdateImage = (type: 'hero' | 'hotel' | 'restaurant' | 'itinerary', index: number, imageData: string) => {
    setQuotation(prev => {
      if (!prev) return null;
      ...
      switch (type) {
        case 'hero':
          updated.heroImage = imageData;
          break;

        case 'hotel':
          if (updated.hotels && updated.hotels[index]) {
            updated.hotels = [...updated.hotels];
            updated.hotels[index] = { ...updated.hotels[index], image: imageData };
          }
          break;

        case 'restaurant':
          if (updated.restaurants && updated.restaurants[index]) {
            updated.restaurants = [...updated.restaurants];
            updated.restaurants[index] = { ...updated.restaurants[index], image: imageData };
          }
          break;

        case 'itinerary':
          if (updated.itinerary && updated.itinerary[index]) {
            updated.itinerary = [...updated.itinerary];
            updated.itinerary[index] = { ...updated.itinerary[index], image: imageData };
          }
          break;
      }
