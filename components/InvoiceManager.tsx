...
import { Plus, Search, FileText, Calendar, Trash2, Edit2, Download, ChevronLeft, Printer, CheckCircle, ArrowRight, ShoppingBag, Building2, Truck, CreditCard, Banknote, MapPin, Mail, Phone, Hash, Upload, Image as ImageIcon, History, List } from 'lucide-react';
...
const InvoiceManager: React.FC<InvoiceManagerProps> = ({ invoices, customers, onAdd, onUpdate, onDelete }) => {
  ...
  const createNewInvoice = () => {
    const newInv: Invoice = {
      ...
      items: [
        { 
            id: '1', description: 'Flight Ticket + Visa Booking', particulars: '', hsnSac: '998552', qty: 1, rate: 0, 
  ...
  const addItem = () => {
    ...
    setActiveInvoice({
      ...
      items: [...activeInvoice.items, { 
          id: Date.now().toString(), description: '', particulars: '', hsnSac: '', qty: 1, rate: 0, 
          igst: 0, cgst: 0, sgst: 0,
          igstRate: 0, cgstRate: 0, sgstRate: 0
      }]
    });
  ...
  const calculateTotals = (items: InvoiceItem[]) => {
    ...
    items.forEach(item => {
      ...
      subTotal += lineTotal;
      // Calculate Tax based on manual Amounts entered by user (Values, not rates)
  ...
  const downloadInvoicePDF = async () => {
    ...
    setTimeout(async () => {
        try {
            ...
            pdf.save(`${activeInvoice?.invoiceNumber || 'Invoice'}.pdf`);
        } catch (e) {
  ...
  if (view === 'editor' && activeInvoice) {
    ...
    return (
        <div className="w-full h-full flex flex-col bg-slate-50/50 font-sans overflow-hidden">
            ...
            <div className="bg-white border-b border-slate-200 px-6 py-4 flex flex-col md:flex-row md:items-center justify-between shrink-0 gap-4">
                <h1 className="text-2xl font-serif font-bold text-slate-900 flex items-center gap-2">
                    <FileText className="w-6 h-6 text-slate-700"/> Invoice Manager
                </h1>
                
                <div className="flex bg-slate-100 p-1 rounded-xl">
                    <button 
                        onClick={() => setView('list')} 
                        className="flex items-center gap-2 px-5 py-2.5 rounded-lg text-sm font-bold text-slate-500 hover:text-slate-700 hover:bg-slate-200/50 transition-all"
                    >
                        <History className="w-4 h-4"/>
                        Invoice History
                    </button>
                    <button 
                        className="flex items-center gap-2 px-5 py-2.5 rounded-lg text-sm font-bold bg-white text-slate-900 shadow-sm ring-1 ring-slate-200 transition-all"
                    >
                        <Plus className="w-4 h-4"/>
                        Editing: {activeInvoice.invoiceNumber}
                    </button>
                </div>
            </div>
                ...
                <div className="w-1/2 md:w-5/12 h-full flex flex-col border-r border-slate-200 bg-white overflow-y-auto">
                    <div className="p-4 border-b border-slate-100 flex items-center justify-between sticky top-0 bg-white/90 backdrop-blur z-10">
                        ...
                        <div className="flex gap-2">
                            <button onClick={downloadInvoicePDF} disabled={isExporting} className="flex items-center gap-2 px-3 py-2 text-slate-600 hover:text-blue-700 bg-slate-50 hover:bg-blue-50 rounded-lg border border-slate-200 transition-colors disabled:opacity-50">
                                {isExporting ? <div className="animate-spin rounded-full h-4 w-4 border-b-2 border-slate-600"></div> : <Download className="w-4 h-4" />}
                                <span className="text-xs font-bold">{isExporting ? 'Generating...' : 'Download PDF'}</span>
                            </button>
                            <button onClick={handleSave} className="bg-slate-900 text-white px-4 py-2 rounded-lg text-sm font-bold shadow-sm hover:bg-slate-800">
                                Save Invoice
                            </button>
                        </div>
                    </div>
                                    ...
                                    <div className="grid grid-cols-8 gap-2">
                                        ...
                                        <div className="col-span-1">
                                            ...
                                            <input 
                                                type="number"
                                                value={item.igst}
                                                onChange={(e) => updateItem(idx, 'igst', Number(e.target.value))}
                                                className="w-full p-1 bg-white border-b border-slate-200 focus:border-blue-500 outline-none text-sm text-slate-900 placeholder-slate-400"
                                            />
                                        </div>
                                        ...
                                        <div className="col-span-1">
                                            ...
                                            <input 
                                                type="number"
                                                value={item.cgst}
                                                onChange={(e) => updateItem(idx, 'cgst', Number(e.target.value))}
                                                className="w-full p-1 bg-white border-b border-slate-200 focus:border-blue-500 outline-none text-sm text-slate-900 placeholder-slate-400"
                                            />
                                        </div>
                                        ...
                                        <div className="col-span-1">
                                            ...
                                            <input 
                                                type="number"
                                                value={item.sgst}
                                                onChange={(e) => updateItem(idx, 'sgst', Number(e.target.value))}
                                                className="w-full p-1 bg-white border-b border-slate-200 focus:border-blue-500 outline-none text-sm text-slate-900 placeholder-slate-400"
                                            />
                                        </div>
                                    </div>
                        ...
                        <div className="pt-4 border-t border-slate-200">
                            ...
                            <div className="flex items-center gap-4">
                                ...
                                {activeInvoice.signatureImage && (
                                    <img src={activeInvoice.signatureImage} alt="Sig" className="h-16 border border-slate-200 rounded object-contain" />
                                )}
                            </div>
                        </div>
                    </div>
                    ...
                    >
                        ...
                        <div className="p-8 pt-4">
                            ...
                            <div className="space-y-3">
                                ...
                                {/* Detailed Tax Breakdown (Values with Labels from Rate inputs) */}
                            ...
                            <div className="text-right">
                                 {activeInvoice.signatureImage && (
                                     <img 
                                        ...
                                        className="h-40 w-64 object-contain mb-4 ml-auto block" 
                                     />
                                 )}
