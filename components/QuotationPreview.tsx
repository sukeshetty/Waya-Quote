...
// Shows when no image URL exists or when image fails to load.
// Lets the user click to upload the correct image.
// =====================================================
...
const ImageWithUpload: React.FC<ImageWithUploadProps> = ({
  src,
  alt,
  ...
  const showPlaceholder = !currentSrc || currentSrc.trim() === "" || imageError;

  if (showPlaceholder) {
    return (
      <div
        className={`${placeholderClassName} bg-gradient-to-br from-slate-100 to-slate-200 flex flex-col items-center justify-center cursor-pointer group hover:from-slate-200 hover:to-slate-300 transition-all duration-300 relative`}
        onClick={handleClick}
      >
        <input
          ref={fileInputRef}
        ...
        <div className="flex flex-col items-center space-y-3 p-6 text-center">
          <div className="w-14 h-14 rounded-full bg-white shadow-md flex items-center justify-center group-hover:shadow-lg group-hover:scale-110 transition-all duration-300">
            <Upload className="w-6 h-6 text-orange-500" />
          </div>
          <div>
            <p className="text-sm font-semibold text-slate-600 group-hover:text-slate-800">{placeholderText}</p>
            <p className="text-xs text-slate-400 mt-1">Click to add the correct photo</p>
          </div>
        </div>
        {/* Dashed border overlay */}
        <div className="absolute inset-2 border-2 border-dashed border-slate-300 rounded-lg pointer-events-none group-hover:border-orange-400 transition-colors"></div>
      </div>
    );
  }
  ...
  return (
    <div className="relative group/img">
      ...
      {onImageUpload && (
        <div
          className="absolute inset-0 bg-black/0 group-hover/img:bg-black/40 transition-all duration-300 flex items-center justify-center cursor-pointer opacity-0 group-hover/img:opacity-100"
          onClick={handleClick}
        >
          <input
            ref={fileInputRef}
            ...
            className="hidden"
          />
          <div className="bg-white/90 backdrop-blur px-4 py-2 rounded-full flex items-center space-x-2 shadow-lg transform scale-90 group-hover/img:scale-100 transition-transform">
            <Upload className="w-4 h-4 text-orange-500" />
            <span className="text-sm font-medium text-slate-700">Replace Image</span>
          </div>
        </div>
      )}
    </div>
...
const QuotationPreview: React.FC<QuotationPreviewProps> = ({ data, loading, id, onUpdatePrice, onUpdateImage }) => {
      ...
      <div className="w-full h-screen flex flex-col items-center justify-center bg-slate-50 relative overflow-hidden">
        ...
        <div className="z-10 text-center space-y-8 p-12 bg-white/40 backdrop-blur-xl rounded-3xl border border-white/50 shadow-2xl">
          ...
          <div>
            ...
            <p className="text-slate-600 mt-2">Searching for real photos & live details</p>
          </div>
        </div>
      </div>
    );
      ...
      <div id={id} className="bg-slate-100 min-h-screen font-sans text-slate-800 pb-20">
        ...
        <div className={CONTAINER_CLASS}>
          <div className="bg-white rounded-xl shadow-sm -mt-6 mb-6 flex border border-slate-200 overflow-hidden">
            <div className="flex-1 py-4 text-center border-b-2 border-blue-600 font-bold text-blue-700 bg-blue-50">Recommended</div>
            <div className="flex-1 py-4 text-center border-b-2 border-transparent text-slate-500 hover:bg-slate-50">Cheapest</div>
            <div className="flex-1 py-4 text-center border-b-2 border-transparent text-slate-500 hover:bg-slate-50">Fastest</div>
          </div>
              ...
              <div key={idx} className="bg-white rounded-xl shadow-sm border border-slate-200 p-6 hover:shadow-md transition-shadow">
                <div className="flex flex-col md:flex-row items-center gap-6">
                  ...
                  <div className="w-full md:w-1/6 text-right pl-6 border-l border-slate-100">
                    ...
                    <button className="w-full bg-blue-600 hover:bg-blue-700 text-white font-medium py-2 rounded-lg transition-colors">Select</button>
                  </div>
                </div>
              ...
              <div key={idx} className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden flex flex-col md:flex-row hover:shadow-md transition-shadow group">
                <div className="md:w-1/3 h-64 md:h-auto relative overflow-hidden">
                  <ImageWithUpload
                    ...
                    placeholderClassName="w-full h-full min-h-[256px]"
                    onImageUpload={(imgData) => onUpdateImage?.('hotel', idx, imgData)}
                    placeholderText={`Upload photo of ${hotel.name}`}
                  />
                </div>
                  ...
                  <div>
                    <div className="flex justify-between items-start">
                      ...
                      {hotel.rating && (
                        <div className="flex flex-col items-end">
                          <div className="flex items-center bg-blue-900 text-white px-2 py-1 rounded-lg font-bold text-sm">{hotel.rating}</div>
                    ...
                    <div className="flex flex-wrap gap-2 my-4">
                      {hotel.amenities.slice(0, 4).map((am, i) => (
                        <span key={i} className="text-xs px-2 py-1 bg-slate-100 text-slate-600 rounded-md border border-slate-200">{am}</span>
                      ))}
                    </div>
                  </div>
  ...
  return (
    ...
    >
      ...
      <div className="relative h-[85vh] w-full group">
        {heroBg ? (
          <>
            <ImageWithUpload
              ...
              placeholderClassName="w-full h-full"
              onImageUpload={(imgData) => onUpdateImage?.('hero', 0, imgData)}
              placeholderText={`Upload hero photo of ${data.destination}`}
            />
          </>
        ) : (
          <ImageWithUpload
        ...
        <div className="absolute inset-0 bg-gradient-to-t from-slate-900 via-slate-900/20 to-transparent opacity-90 pointer-events-none"></div>
        ...
        {/* Hero Content */}
        <div className="absolute bottom-0 left-0 w-full pb-20 z-10 pointer-events-none">
          <div className={CONTAINER_CLASS}>
            <div className="space-y-6">
              ...
              <div className="flex flex-col md:flex-row items-start md:items-center gap-4 pt-4 pointer-events-auto">
        ...
        <div className={`${CONTAINER_CLASS}`}>
          <div className="bg-white/80 backdrop-blur-xl rounded-2xl shadow-xl border-t border-white/40 border-b border-white/20 p-6 grid grid-cols-2 lg:grid-cols-4 gap-6">
            <div className="flex items-center space-x-4">
              <div className="p-3 bg-blue-50 text-blue-600 rounded-xl flex-shrink-0"><Plane className="w-6 h-6" /></div>
              <div>
            ...
            <div className="flex items-center space-x-4">
              <div className="p-3 bg-purple-50 text-purple-600 rounded-xl flex-shrink-0"><Hotel className="w-6 h-6" /></div>
              <div>
            ...
            <div className="flex items-center space-x-4">
              <div className="p-3 bg-emerald-50 text-emerald-600 rounded-xl flex-shrink-0"><Shield className="w-6 h-6" /></div>
              <div>
            ...
            <div className="flex items-center space-x-4">
              <div className="p-3 bg-orange-50 text-orange-600 rounded-xl flex-shrink-0"><Star className="w-6 h-6" /></div>
              <div>
      ...
      <div className={`${CONTAINER_CLASS} space-y-20`}>
        ...
        <section className="flex flex-col gap-12">
          <div className="text-left w-full">
            ...
            <p className="text-lg text-slate-600 leading-relaxed font-light">{data.summary}</p>
          </div>
          <div className="bg-white p-8 rounded-3xl shadow-lg border border-slate-100">
            <h4 className="font-bold text-slate-900 mb-6 flex items-center">
              <CheckCircle className="w-5 h-5 mr-2 text-emerald-500" /> Package Inclusions
            </h4>
            <div className="flex flex-wrap justify-start gap-3">
              {displayInclusions.map((item, i) => (
                <span key={i} className="px-4 py-2 bg-slate-50 hover:bg-slate-100 text-slate-700 text-sm font-semibold rounded-full border border-slate-200 transition-colors cursor-default">{item}</span>
              ))}
            </div>
          </div>
        </section>
                ...
                <div key={idx} className="group relative bg-white rounded-3xl overflow-hidden shadow-lg hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-1">
                  <div className="h-64 w-full relative overflow-hidden">
                    <ImageWithUpload
                      ...
                      placeholderText={`Upload photo of ${hotel.name}`}
                    />
                    <div className="absolute top-4 right-4 bg-white/90 backdrop-blur text-slate-900 px-3 py-1 rounded-full text-xs font-bold shadow-sm z-10">
                      {hotel.roomType}
                    </div>
                    {hotel.rating && (
                      <div className="absolute bottom-4 left-4 flex items-center bg-blue-900/90 text-white px-2 py-1 rounded-lg backdrop-blur-sm z-10">
              ...
              <div key={idx} className="flex gap-4 md:gap-6 group">
                ...
                <div className="flex-1 pb-12">
                  <div className="bg-white rounded-2xl shadow-md border border-slate-100 hover:shadow-lg transition-shadow overflow-hidden">
                    {/* Itinerary day image - shows upload placeholder if missing */}
                    <div className="h-48 w-full relative">
                      <ImageWithUpload
                        src={day.image}
                        alt={day.title}
                        className="w-full h-full object-cover"
                        placeholderClassName="w-full h-full min-h-[192px]"
                        onImageUpload={(imgData) => onUpdateImage?.('itinerary', idx, imgData)}
                        placeholderText={`Upload photo: ${day.title}`}
                      />
                      {day.image && (
                        <>
                          <div className="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent pointer-events-none"></div>
                          <h4 className="absolute bottom-4 left-6 text-xl font-bold text-white shadow-sm z-10">{day.title}</h4>
                        </>
                      )}
                    </div>
                    <div className="p-6">
                        ...
                        {day.activities.map((act, actIdx) => (
                          <li key={actIdx} className="flex flex-col sm:flex-row gap-4 p-4 rounded-xl hover:bg-slate-50 transition-colors border border-transparent hover:border-slate-100">
                            <div className="flex-shrink-0">
                              <span className="inline-block text-xs font-bold text-slate-600 bg-slate-100 px-3 py-1.5 rounded-lg min-w-[80px] text-center">{act.time}</span>
                            </div>
                            <div>
                              <p className="text-slate-700 leading-relaxed text-sm md:text-base">{act.description}</p>
                              {act.location && (
                                <span className="inline-block mt-2 text-xs font-medium text-orange-600 bg-orange-50 px-2 py-1 rounded">📍 {act.location}</span>
                              )}
                            </div>
                          </li>
                        ))}
                ...
                <div key={idx} className="min-w-[300px] md:min-w-[350px] snap-center bg-white rounded-3xl overflow-hidden shadow-md border border-slate-100 flex flex-col">
                  <div className="h-48 relative">
                    <ImageWithUpload
                      ...
                      className="w-full h-full object-cover"
                      placeholderClassName="w-full h-full min-h-[192px]"
                      onImageUpload={(imgData) => onUpdateImage?.('restaurant', idx, imgData)}
                      placeholderText={`Upload photo of ${rest.name}`}
                    />
                    <div className="absolute bottom-4 left-4 bg-black/60 backdrop-blur-md text-white px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider z-10">
                      {rest.cuisine}
                    </div>
                  </div>
                  <div className="p-6 flex-1 flex flex-col">
                    ...
                    <button className="w-full py-2 rounded-xl bg-slate-50 text-slate-600 text-xs font-bold hover:bg-slate-100 transition-colors">View Menu</button>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {data.travelTips?.map((tip, i) => (
              <div key={i} className="flex items-start p-4 bg-white/60 rounded-xl">
                <div className="w-6 h-6 rounded-full bg-orange-100 flex items-center justify-center text-orange-600 font-bold text-xs mr-3 flex-shrink-0">{i + 1}</div>
          ...
          <svg viewBox="0 0 64 64" fill="none" xmlns="http://www.w3.org/2000/svg" className="w-full h-full">
            <defs>
              ...
              <linearGradient id="footer_grad_2" x1="0" y1="0" x2="64" y2="64" gradientUnits="userSpaceOnUse">
                <stop stopColor="#EC4899" /><stop offset="1" stopColor="#F472B6" />
              </linearGradient>
            </defs>
